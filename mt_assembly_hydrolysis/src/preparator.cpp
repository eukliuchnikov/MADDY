...
void initParameters(int argc, char* argv[]){
    ...
    par.mt_stride = getLongIntegerParameter(PARAMETER_MT_STRIDE, -1);
...
void read_PDB(const char* filename_xyz, const char* filename_ang){
    ...
    for(int traj = 0; traj < par.Ntr; traj++){
        for(int i = 0; i < par.Ntot; i++){
            if (pdb.atoms[i].occupancy == 1.0) {
                top.gtp[i + traj*par.Ntot] = 1.0;     
            } else {
                top.gtp[i + traj*par.Ntot] = 0.0;
            }
        }
    } 
    ...
    for(int traj = 0; traj < par.Ntr; traj++){
        for(i = 0; i < par.Ntot; i++){
            if(pdb.atoms[i].chain == 'X' && pdb.atoms[i].z > 590.0){
