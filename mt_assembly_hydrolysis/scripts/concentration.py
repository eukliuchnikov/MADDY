...
if len(argv) != 4:
		...
		print "Volume = %f [nm^3]\n " % V
		print "Free particles # (dimers) = %d \n" % int(C * V) 
		print "1) Launch python make_mt_cncntr.py %d extra#\n" % ( 2 * (int(C * V / 13) + 1))
		
print """2) Compile with  LJ and REPULSIVE, no MORSE, no ASSEMBLY, no MT_LENGTH, no CONCentration. 
3) Simulate *.pdb files for 10 million steps and fix = 1, runnum = 1;
4) Take result.pdb as initial structure	
5) Compile with LJ, CONC, MORSE, ASSEMBLY, MT_LENGTH
6) Simulate trajectory with any runnum, fix = 1 and so on."""
